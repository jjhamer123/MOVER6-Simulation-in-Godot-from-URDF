...
func fix_meshes(robot_node: Node3D) -> void:
	...
	var target_links := get_compound_links_from_urdf(resolved_path)
	print("ArmRig: Target URDF compound links -> ", target_links)

	var rotated_nodes: Dictionary = {}
	for link_name in target_links:
		var link_nodes := _find_nodes_by_name_part(robot_node, link_name)
		if link_nodes.is_empty():
			push_warning("ArmRig: Could not find node matching link -> " + link_name)
		else:
			for node in link_nodes:
				_rotate_meshes_in_branch(node, rotated_nodes)


func _find_nodes_by_name_part(node: Node, target_name: String) -> Array[Node]:
	var results: Array[Node] = []
	if node.name.to_lower().contains(target_name.to_lower()):
		results.append(node)
	for child in node.get_children():
		results.append_array(_find_nodes_by_name_part(child, target_name))
	return results


func _rotate_meshes_in_branch(node: Node, rotated_nodes: Dictionary) -> void:
	if node in rotated_nodes:
		return

	if node is Node3D:
		var name_lower := node.name.to_lower()
		# Target visual mesh nodes while excluding collision nodes
		if (node is MeshInstance3D or name_lower.begins_with("visual") or "mesh" in name_lower) and not "collision" in name_lower:
			node.rotate_object_local(Vector3.RIGHT, deg_to_rad(90.0))
			rotated_nodes[node] = true
			print("ArmRig: Rotated +90° X on node -> ", node.name, " [", node.get_class(), "] under ", node.get_parent().name)
			return

	for child in node.get_children():
		_rotate_meshes_in_branch(child, rotated_nodes)
